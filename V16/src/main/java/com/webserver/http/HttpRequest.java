...
public class HttpRequest {
    ...
    private void parseUri() {
           ...
           if( uri.contains("?")){
                ...
                */
               try {
                   // String  decode(String string1,String enc)
                   //URLDecoder中的方法，将一个16进制的String1按照enc（一般使用“utf-8”）的字符集类型转换，并返回一个String
                   //这里使用时将form表提交上来的%xx转换为中文  这里decode需要处理一个异常
    ...
    private void parseContent(){
        System.out.println("HttpRequest:开始解析消息正文...");
            //POST请求会包含消息正文
        // boolean equalsIgnoreCase（String string） 时忽略大小写的
        if ("post".equalsIgnoreCase(method)){
            ...
            if(len!=null){
                ...
                //根据消息头Content-Type了解浏览器发送过来的正文是什么病进行对应的处理
                String type=headers.get("Content-Type");
                if (type!=null){
                    //判断是否为form表单提交的数据
                    if("application/x-www-form-urlencoded".equalsIgnoreCase(type)){
                        try {
                            //上述类型的正文实际上就是字符串，内容与GET提交时？右侧内容一样
